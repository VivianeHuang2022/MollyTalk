...
const modules = import.meta.glob([
  "../../../assets/avatars/*",
  "!../../../assets/avatars/self.png",
]);
const avatarList = ref<string[]>(
  Object.keys(modules).map((path) =>
    path.replace("../../../assets/avatars/", "")
  )
);
const currentAvatarIndex = ref((Math.random() * avatarList.value.length) | 0);

const store = useConversationStore();
const { ssmlToSpeak, isSynthesizing, isPlaying } = useSpeechService({
  isFetchAllVoice: false,
});
const allLanguages = computed(() =>
  [...new Set(allVoices.map((v) => v.locale))].filter((l) =>
    Object.keys(supportLanguageMap).includes(l)
  )
);
const desc = ref("");
const name = ref("");
const rate = ref("1.0");
const previewText = ref("MollyTalk is awesome!");
const presets = ref(
    ...
    allVoices.forEach((v) => {
      if (v.locale === item) {
        children.push({
          ...
          children:
            v.styleList?.map((x) => ({ label: voiceStyleMap[x], value: x })) ||
            [],
        });
      }
    });
...
const randomAvatar = getAvatarUrl(
  avatarList.value[(Math.random() * avatarList.value.length) | 0]
); // 随机默认选择一个头像
...
const changeAvatar = () => {
  currentAvatarIndex.value =
    avatarList.value.length - 1 === currentAvatarIndex.value
      ? 0
      : currentAvatarIndex.value + 1;
};
const previewSpeech = () => {
  ssmlToSpeak(previewText.value, {
    voice: selectVoiceName.value,
...
<template>
  <div class="wrapper" flex="~ col gap-3" items-center>
    ...
    <div class="ml-[-30px] relative center-y">
      <div flex>
        <label for="">voice test</label>
        <input
          v-model="previewText"
          placeholder="输入文字预览语音"
          type="text"
        />
      </div>
      <div absolute left-73>
        <button
          v-if="!isSynthesizing && !isPlaying"
          cursor-pointer
    ...
    <div center-y text-sm text-gray-500>
      ...
      click change photo to choose your photos
    </div>
    <button
      w-20
      btn
