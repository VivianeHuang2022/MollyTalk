...
const defaultConversations = [
  {
    ...
    chatMessages: [
      {
        role: "system",
        content: generatePrompt("English", "Anthony"),
      },
    ],
  },
] as const;
...
export const useConversationStore = defineStore("conversation", {
  ...
  getters: {
    ...
    allPeople(state) {
      return state.conversations.map((x) => ({
        key: x.key,
        desc: x.desc,
        avatar: x.avatar,
        name: x.name,
    ...
    getConversationsByCurrentProps(state) {
      return <T extends keyof Conversation>(props: T): Conversation[T] =>
        state.conversations.find((x) => x.key === state.currentKey)![props];
    },
  },
  actions: {
    ...
    addConversation(
      conversation: Omit<Conversation, "chatMessages">,
      systemPrompt?: string
    ) {
        ...
        chatMessages: [
          {
            ...
            content: systemPrompt
              ...
              : generatePrompt(conversation.language, conversation.name),
          },
        ],
      });
    },
    ...
    deleteConversation(key: Key) {
      if (this.conversations.length === 1) return alert("至少保留一个会话");

      this.conversations = this.conversations.filter((x) => x.key !== key);

      if (this.currentKey === key) this.currentKey = this.conversations[0]?.key;
    },
  },
});
